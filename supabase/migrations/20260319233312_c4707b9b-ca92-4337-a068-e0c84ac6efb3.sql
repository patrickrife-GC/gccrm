...
DECLARE
  ...
  random_idx int;
  speaking_messages text[];
BEGIN
  -- icp_segment overrides (placed before existing logic)
  IF NEW.icp_segment = 'Identity Shifter' THEN
    NEW.outreach_angle := 'story_to_content';
    NEW.suggested_message := 'You''re in the middle of a real transition — the kind of story that resonates deeply when it''s told right. Have you thought about capturing it as you go?';
    RETURN NEW;
  END IF;

  IF NEW.icp_segment = 'Stuck Scaler' THEN
    intents := array_append(intents, 'ground_control');
    NEW.outreach_angle := 'services_intro';
    NEW.suggested_message := 'You''re past the scrappy stage but the systems haven''t caught up yet — that''s exactly the gap I work in. Worth a quick conversation?';
  END IF;

  IF NEW.icp_segment = 'Technical Builder' THEN
    intents := array_append(intents, 'ideoloop');
    NEW.outreach_angle := 'story_to_content';
    NEW.suggested_message := 'You''re building something genuinely interesting — have you thought about turning the process into content? Talking through it is usually all it takes.';
  END IF;

  IF NEW.icp_segment = 'Busy Expert' THEN
    intents := array_append(intents, 'ground_control');
    NEW.outreach_angle := 'newsletter_feature';
    NEW.suggested_message := 'Your perspective on this space is sharp — thinking it could make a strong feature for the Ground Control newsletter. Open to a quick conversation?';
  END IF;
  ...
  -- Auto-assign suggested_outreach_type (only if not already set by icp_segment)
  IF NEW.suggested_message IS NULL THEN
  ...
  -- Auto-assign desired_outcome (only for ground_control)
  IF 'ground_control' = ANY(intents) THEN
    CASE NEW.gc_outreach_type
      WHEN 'ghostwriting' THEN NEW.desired_outcome := 'revenue';
      WHEN 'social_growth' THEN NEW.desired_outcome := 'revenue';
      WHEN 'website' THEN NEW.desired_outcome := 'revenue';
      WHEN 'lead_gen' THEN NEW.desired_outcome := 'revenue';
      WHEN 'outbound_strategy' THEN NEW.desired_outcome := 'revenue';
      WHEN 'general_services' THEN NEW.desired_outcome := 'revenue';
      WHEN 'speaking' THEN NEW.desired_outcome := 'audience_growth';
      WHEN 'partnership' THEN NEW.desired_outcome := 'leverage';
      ELSE NEW.desired_outcome := NULL;
    END CASE;
  ELSE
    NEW.desired_outcome := NULL;
  END IF;

  -- Auto-assign suggested_message (only if not already set by icp_segment)
  IF NEW.suggested_message IS NULL THEN
    IF 'ground_control' = ANY(intents) THEN
      CASE NEW.gc_outreach_type
        ...
        WHEN 'speaking' THEN
          speaking_messages := ARRAY[
            'Really like what you''re building — I''d love to contribute something valuable to your audience. Have you thought about bringing in speakers or guest sessions?',
            'I''ve been sharing a lot around building + systems lately — would love to do a session for your audience if you''re open to it.'
          ];
          random_idx := 1 + floor(random() * 2)::int;
          NEW.suggested_message := speaking_messages[random_idx];

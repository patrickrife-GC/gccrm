...
ALTER TABLE public.contacts ADD COLUMN service_type text;
...
BEGIN
  ...
  -- Auto-assign service_type (only for ground_control)
  IF 'ground_control' = ANY(intents) THEN
    IF lower_title ~ '(writer|editor|journalist|copywriter|author)' THEN
      NEW.service_type := 'ghostwriting';
    ELSIF lower_title ~ '(social|community|influencer|creator)' OR lower_company ~ '(social|community)' THEN
      NEW.service_type := 'social_growth';
    ELSIF lower_title ~ '(web|developer|engineer|designer|ux|ui|frontend|backend|fullstack)' OR lower_company ~ '(web|dev|design|digital|agency)' THEN
      NEW.service_type := 'website';
    ELSIF lower_title ~ '(sales|business development|bdr|sdr|account executive|outbound)' THEN
      NEW.service_type := 'outbound_strategy';
    ELSIF lower_title ~ '(growth|demand|lead|acquisition|performance|paid)' THEN
      NEW.service_type := 'lead_gen';
    ELSE
      NEW.service_type := 'general_services';
    END IF;
  ELSE
